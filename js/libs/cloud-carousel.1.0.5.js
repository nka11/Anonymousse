...
//		func = function(){return;ctx.updateAll();} ;
		...
		this.checkImagesLoaded = function()
		{
			...
			for(i=0;i<images.length;i++) {
				$img = $(images[i]);
				if ( (images[i].width === undefined) || (  ($img.data('complete') !== undefined) && (!$img.data('complete') == true)  ))
				{
					return;					
				}				
			}
			for(i=0;i<images.length;i++) {	
				$img = $(images[i]);
				ctx.items.push( new Item( images[i], options ) );	
				$img.data('itemIndex',i);
			}
		...
		this.addImage = function(img) {
			function appendImage(img) {
				var $img = $(img);
				$img.data('itemIndex', ctx.items.length());
				$container.append($img);
				ctx.items.push( new Item( img, options ) );
				this.showFrontText();
				this.autoRotate();
				this.updateAll();
			}
			if ( (img.width === undefined) || ( ($img.data('complete') !== undefined) && (!$img.data('complete') == true)  )) {
				img.onloadend = function() {appendImage(img)};
			} else {
				appendImage(img);
			}
